...
public class WordsParser {
	...
	static final String TAG = "WordParser";
	...
	public static List<WordsParserItem> parse(InputStream inputStream) {
		final List<WordsParserItem> item = new ArrayList<WordsParserItem>();
		...
		card.setEndElementListener(new EndElementListener() {
			public void end() {
				item.add(currentCard.copy());
			}
		});

		card.getChild(WORD).setEndTextElementListener(
		...
		examples.getChild(EXAMPLE).setEndTextElementListener(
				new EndTextElementListener() {
					public void end(String body) {
		...
		translations.getChild(WORD).setEndTextElementListener(
				new EndTextElementListener() {
					public void end(String body) {
		...
		try {
			Xml.parse(inputStream, Xml.Encoding.UTF_16,

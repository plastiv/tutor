...
/*
 * Пример файла xml который здесь разбирается
 * 
...
public class FeedParser {
	...
	private InputStream feedInputStream;
	
	public FeedParser(InputStream inputStream){
		this.feedInputStream = inputStream;
	}

	public List<Cards> parse() {
		final List<Cards> cards = new ArrayList<Cards>();
		final Cards currentCard = new Cards();
		...
		try {
			Xml.parse(this.feedInputStream, Xml.Encoding.UTF_16, root.getContentHandler());
		} catch (Exception e) {

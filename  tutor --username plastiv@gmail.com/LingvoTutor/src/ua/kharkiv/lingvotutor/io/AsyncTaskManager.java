...
public final class AsyncTaskManager implements IProgressTracker, OnCancelListener {
    ...
    private DownloadTask mAsyncTask;
    ...
    public void setupTask(DownloadTask asyncTask, String filename) {
	// Keep task
	mAsyncTask = asyncTask;
	// Wire task to tracker (this)
	mAsyncTask.setProgressTracker(this);
	// Start task
	mAsyncTask.execute(filename);
    }
	...
	if (instance instanceof DownloadTask) {
	    mAsyncTask = (DownloadTask) instance;
	    mAsyncTask.setProgressTracker(this);
	}
    }
